...
output "irmai_service_external_ip" {
  description = "The external IP address of the irmaiauth-demo-module-service LoadBalancer. May take a few minutes to become available."
  ...
  value       = try(kubernetes_service.irmaiauth_demo_service.status[0].load_balancer[0].ingress[0].ip, "pending")
}
